...
def file_wr(filename, infos):
    ...
    csv_counter += 1
    name = filename + '' + str(csv_counter) + '.csv'
    fp = open(name, 'wb+')
    fp.write(infos)
    fp.close()
    print("Finished")
...
for country in country_codes:
    ...
    for j in range(len(list_of_available_pollutants[country])):
        # if pollutant_codes[list_of_available_pollutants[country][j]] != 'SO2' and pollutant_codes[list_of_available_pollutants[country][j]] != 'CO' and pollutant_codes[list_of_available_pollutants[country][j]] != 'NO2':
        #     continue
        ...
        try:
            reqs = requests.get(
                ...
                country + '&CityName=&Pollutant=' + str(
                    list_of_available_pollutants[country][j]) + '&Year_from=2016&Year_to=2016&Station'
        ...
        for link in soup.find_all('a')[:50]:
            ...
            try:
                ...
                csvPath = file_wr(path, html)
                with open(csvPath, 'rb') as f:
                    info = chardet.detect(f.read())
                    pdData = pd.read_csv(csvPath, sep=',', usecols=cols, encoding=info.get('encoding'))
                    pdData = pdData.sort_values(by=['DatetimeBegin'])
                    pdData = pdData.iloc[12::24, :]
                    pdData.to_csv(csvPath, index = False)
